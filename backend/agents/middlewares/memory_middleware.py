...
from agents.memory.prompt import format_memory_for_injection
...
class MemoryMiddleware(AgentMiddleware[AgentState]):
    ...
    def _inject_memory(self, state: AgentState) -> dict | None:
        """Inject memory into the conversation as a system message."""
        config = get_app_config()
        if not config.memory.enabled or not config.memory.max_injection_tokens:
            return None

        memory = get_memory_data()
        formatted = format_memory_for_injection(memory, config.memory.max_injection_tokens)
        if not formatted:
            return None

        memory_message = "<memory>\n" + formatted + "\n</memory>"
        return {"messages": [SystemMessage(content=memory_message)]}
