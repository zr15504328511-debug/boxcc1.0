...
- Write all `checklist_draft.title` fields in Simplified Chinese.
- Write `checklist_self_check.issues` and `checklist_self_check.fixes` in Simplified Chinese.
...
- Preserve the key conclusions from every selected worker instead of collapsing everything into a short abstract.
- When worker outputs are detailed, the final answer must still surface department-level findings, not only a one-paragraph summary.
- If there was critic-directed rework, explicitly reflect the corrected final position rather than the rejected draft.
""".strip()
